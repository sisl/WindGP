...
for h in [150]
    append!(X, [[i, j, Float64(h)] for j in 0.0:grid_dist:(ny-1)*grid_dist for i in 0.0:grid_dist:(nx-1)*grid_dist])
...
gp = fetchOptimizedGP(X_gp, Y, d, NN, opt_init0)
...
Y_star = rand(gp, gp.kernel, Xs_gp, numNeighbors, grid_dist_Xs)

p1 = heatmap(reshape(Y, (nx,ny)))

nx_star = length(0.0:grid_dist_Xs:(nx-1)*grid_dist)
ny_star = length(0.0:grid_dist_Xs:(ny-1)*grid_dist)

# predicted h=150
p2 = heatmap(reshape(Y_star, (nx_star,ny_star)))